...
def updateDaumMovieTVSeries(metadata, media):
    ...
    try:
        ...
        if html:
            SetTvShow(html, metadata)
            SetTvShowActors(html)
            SetTvShowSeason(html)
    except Exception, e:
        Log.Debug(repr(e))
        pass

def SetTvShow(html , metadata):
    items = []
    ...
    if match:
        ...
        items.append({"title":title, "airdate":airdate})
  
    seriesNumber = html.xpath('count(//div[@id="tvpColl"]//div[@id="series"]/ul/li/a/text())')
    for i in range(1, int(seriesNumber)+1):
        title = html.xpath('//div[@id="tvpColl"]//div[@id="series"]/ul/li[' + str(i) + ']/a')[0].text.strip()
        ...
        if match:
            ...
            items.append({"title":title, "airdate":airdate})
    items = sorted(items, key=lambda k: k['airdate'], reverse=True)
    #TV show 기본 메타정보
    metadata.genres.clear()
    metadata.countries.clear()
    metadata.roles.clear()
    try: metadata.title = items[0]['title']
    except: pass
    metadata.original_title = ''
    try: metadata.studio = html.xpath('//div[@id="tvpColl"]//div[@class="txt_summary"]/span[1]')[0].text.strip()
    except: pass
    try: metadata.originally_available_at = items[0]['airdate']
    except: pass
    try: metadata.genres.add(html.xpath('//div[@id="tvpColl"]//dt[contains(.,"' + u'장르' + '")]//following-sibling::dd')[0].text.strip().split('(')[0].strip())
    except: pass
    try: metadata.summary = String.DecodeHTMLEntities(String.StripTags(html.xpath('//div[@id="tvpColl"]//dt[contains(.,"' + u'소개' + '")]//following-sibling::dd')[0].text).strip())
    except: pass
    try:
        poster_url = 'https:' + html.xpath('//div[@id="tvpColl"]//div[@class="info_cont"]/div[@class="wrap_thumb"]//img/@src')[0].strip()
        Log.Info(poster_url)
        if poster_url:
           poster = HTTP.Request(poster_url)
           try: metadata.posters[poster_url] = Proxy.Media(poster)
...
def SetTvShowActors(html):
    pass
def SetTvShowSeason(html):
    pass
def SetTvShowEpisode(html):
    pass
def SetTvShowCrews(html):
    pass
def updateDaumMovieTVSeries1(metadata, media, programIds):
...
# JSON 파일이 있는 경우 기존 메타 정보를 덮어쓴다
    ...
    for season_num, season_json in map(None, season_num_list, series_json_data['programList']):
        crewsdata = []
        if season_json is None:
            if len(series_json_data['programList']) == 1:
                programId = programIds 
                season = metadata.seasons[season_num]
                try:  season.summary = metadata.summary
                except: pass
                try:
                    if poster_url:
                        poster = HTTP.Request(poster_url)
            ...
            try: crewsdata  = season_json['crews']
            except : pass
          #출연진 정보
            ...
            if match:
                ...
                for episodedata in episode_json_data:
                    ...
                    if episodedata['channels'][0]['broadcastDate']:
                        try:
                            episode_date = Datetime.ParseDate(episodedata['channels'][0]['broadcastDate'], '%Y%m%d').date().strftime('%Y-%m-%d')
                        except: pass
                    else :
                        try:
                            episode_date = Datetime.ParseDate(episodedata['channels'][1]['broadcastDate'], '%Y%m%d').date().strftime('%Y-%m-%d')
                        except: pass
                    ...
                    if episode_num:
                        ...
                        if  episode_num != episode_date:
                            try: episode.originally_available_at = Datetime.ParseDate(episode_date).date()
                            except: pass
...
class DaumMovieTVSeriesAgent(Agent.TV_Shows):
    ...
    def update(self, metadata, media, lang):
        ...
        #    episode = metadata.seasons[season_num].episodes[episode_num]
        #    Log.Info(episode_num)
        #    Log.Info(episode.title)
        #    if episode.title is None and episode.summary is None and episode.originally_available_at is None:
        #        Log.Info(episode.title)
